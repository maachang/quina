...
public class Jwt {
	...
	static {
		String s = null;
		try {
			s = Base64.encode("{\"alg\":\"HS256\",\"type\":\"JWT\"}".getBytes("UTF8"));
		} catch (Exception e) {
		}
		JWT_HEADER = s;
	}
			...
			String value = new StringBuilder(JWT_HEADER).append(".")
				.append(Base64.encode(
					(payload instanceof String ? (String) payload :
						Json.encode(payload)).getBytes("UTF8")))
				.toString();
			return new StringBuilder(value).append(".")
				.append(signature(key.getBytes("UTF8"), value.getBytes("UTF8")))
				.toString();
	...
	public static final boolean validate(String key, String jwt) {
		...
		try {
			return jwt.substring(p + 1).equals(
				signature(key.getBytes("UTF8"), jwt.substring(0, p).getBytes("UTF8")));
		} catch (QuinaException qe) {
			throw qe;
		} catch (Exception e) {
			throw new QuinaException(e);
