...
public class NioBuffer {
	 ...
	 */
	public NioBuffer(boolean closeToClean) {
	 ...
	 *            [true]を設定した場合、クローズ処理時に情報も破棄します.
	 */
	public NioBuffer(int size, boolean closeToClean) {
		if (size <= MIN_LENGTH) {
	...
	public void write(int b) {
		if (closeFlag) {
			throw new NioException("Already closed.");
		}
	...
	private static final class NioBufferByInputStream extends InputStream {
		...
		protected NioBufferByInputStream(NioBuffer buffer) {
			if(buffer == null) {
	...
	private static final class NioBufferByOutputStream extends OutputStream {
		...
		public NioBufferByOutputStream(NioBuffer buffer) {
			if(buffer == null) {
				throw new NullPointerException();
			}
			this.buffer = buffer;
